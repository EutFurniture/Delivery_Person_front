...
export default function UpdateCashOnDelivery() {
 ...
 const [newPayment_status,setNewPayment_status]=useState(0);
  ...
  const UpdateConDelivery = (order_id) => {
    axios.put(`http://localhost:3001/confirmcashondelivery/${order_id}`, {payment_status:newPayment_status,total_price , advance_price, order_id:order_id}).then(
      (response) => {
        setReturnList(Dt.map((record) => {
          return record.id === order_id ? {order_id: record.order_id,advance_price:record.advance_price, total_price: record.total_price , payment_status: record.payment_status,  o_status:newPayment_status} : record
